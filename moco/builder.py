...
class KCL(nn.Module):
    ...
    https://arxiv.org/abs/1911.05722
    """
    def __init__(self, base_encoder, pretrained = False, n_cls = 1000, dim=128, K=65536, m=0.999, T=0.07, mlp=False, num_positive=0, targeted=False, tr=1,
        ...
        if targeted:
            ...
            target_file = Path(optimal_target_dir) / f"optimal_{self.n_cls}_{dim}_analytical.npy"
            optimal_target = np.load(target_file)
        else:
        ...
        self.register_buffer("queue_feature", torch.randn(dim_mlp, K))
        self.queue_feature = nn.functional.normalize(self.queue_feature, dim=0)
    ...
    @torch.no_grad()
    def _dequeue_and_enqueue(self, keys, labels, feature=None):
        # gather keys before updating queue
        # keys = concat_all_gather(keys)
        ...
        if feature is not None:
            self.queue_feature[:, ptr:ptr + batch_size] = feature.T
        self.queue[:, ptr:ptr + batch_size] = keys.T
    ...
    def forward(self, im_q, im_k, im_labels, eval=False, wight=False):
        """
        Input:
            im_q: a batch of query images
            im_k: a batch of key images
        ...
        if self.targeted:
            ...
            with torch.no_grad():
                ...
                for one_label in torch.unique(im_labels_all):
                    ...
                    self.class_centroid[one_label] = 0.9*self.class_centroid[one_label] + 0.1*class_centroid_batch
                    self.class_centroid[one_label] = F.normalize(self.class_centroid[one_label], dim=0)
                # self.optimal_target_unique = self.class_centroid.transpose(0,1)
        ...
        if wight:
            # class-averaging
            mask_wight1 = torch.ones(logits.shape[0],self.queue_labels.shape[1]).cuda()
            mask_wight2 = torch.ones(logits.shape[0]).cuda()
            wight = torch.zeros(self.n_cls).cuda()
            for i in range(self.n_cls):
                wight[i] = (self.queue_labels==i).sum()
                if wight[i] != 0:
                    for j in range(logits.shape[0]):
                        if im_labels[j]==i:
                            mask_wight1[j,(self.queue_labels==i).squeeze()] = wight[i]-1
                        else:
                            mask_wight1[j,(self.queue_labels==i).squeeze()] = wight[i]
                    mask_wight2[(im_labels==i).squeeze()] = wight[i]-1
            #Taregte时，有问题
            if self.targeted:
                mask_wight3 = torch.ones(logits.shape[0],target_labels.shape[1]).cuda()
                mask_wight = 1.0/torch.cat([mask_wight1, mask_wight2.unsqueeze(dim=1), mask_wight3],dim=1)
            else:
                mask_wight = 1.0/torch.cat([mask_wight1, mask_wight2.unsqueeze(dim=1)],dim=1)
            # mask_wight = (self.queue_labels.shape[0]+1)/torch.cat([mask_wight1, mask_wight1],dim=1)
            # 测试当mask_wight=1，与原loss计算结果是否相同
            log_prob_wight = (logits.exp()/(logits.exp()*mask_wight).sum(1).unsqueeze(1)).log()
            loss_class = - torch.sum((mask_pos_view_class * log_prob_wight).sum(1) / mask_pos_view.sum(1)) / mask_pos_view.shape[0]
            loss_target = - torch.sum((mask_pos_view_target * log_prob_wight).sum(1) / mask_pos_view.sum(1)) / mask_pos_view.shape[0]
        else:
            log_prob = F.normalize(logits.exp(), dim=1, p=1).log()
        ...
        # dequeue and enqueue
        self._dequeue_and_enqueue(k, im_labels, feat_q)
