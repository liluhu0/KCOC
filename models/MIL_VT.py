...
import numpy as np
from timm.models.vision_transformer import VisionTransformer, _cfg, vit_small_patch16_384
from timm.models.swin_transformer import SwinTransformer, _cfg, swin_base_patch4_window12_384_in22k
...
class MILVisionTransformer(VisionTransformer):
    def __init__(self, *args, **kwargs):
        ...
        self.MIL_classifier = nn.Linear(self.L*self.K, self.num_classes)
        ...
        self.MIL_classifier.apply(self._init_weights)
...
class MILVisionTransformer_Distil(VisionTransformer):
    ...
    def forward(self, x):
        ...
        # return vt_out, mil_out, x_patches
        if self.training:
            return (vt_out, dist_out), mil_out, A
        else:
            # during inference, return the average of both classifier predictions
            return (vt_out + dist_out + mil_out) / 3
#######################

class MILSwinVisionTransformer(SwinTransformer):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.dist_token = nn.Parameter(torch.zeros(1, 1, self.embed_dim))
        num_patches = self.patch_embed.num_patches
...
@register_model
def MIL_VT_Distil_small_patch16_384(pretrained=False, **kwargs):
    model = MILVisionTransformer_Distil(
        img_size=384, patch_size=16, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4, qkv_bias=True, 
        distilled=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
        checkpoint = torch.hub.load_state_dict_from_url(
            url="",
...
@register_model
def MIL_VT_Distil_small_patch16_512(pretrained=False, **kwargs):
    model = MILVisionTransformer_Distil(
        img_size=512, patch_size=16, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4, qkv_bias=True,
        distilled=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model.default_cfg = _cfg()
...
@register_model
def vit_small_patch16_512(pretrained=False, **kwargs):
