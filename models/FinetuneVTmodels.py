...
#加载ImageNet上的预训练权重，并去除分类头。或image size不同，则会自动对position embedding进行插值
def ModelPositionEmbeddingFineTune(base_model='MIL_VT_small_patch16_384',  \
                        MODEL_PATH_finetune = None,
                        num_classes=5):
    """Load pretrain weight from distillation model, to train a plain model"""
    ...
    if MODEL_PATH_finetune:
        checkpoint0 = torch.load(MODEL_PATH_finetune, map_location='cpu')
        checkpoint_model = checkpoint0['state_dict']
        # checkpoint_model = rename_pretrain_weight(checkpoint0)
    else:
        model384 = vit_small_patch16_384(pretrained = True)
        checkpoint_model = model384.state_dict()
